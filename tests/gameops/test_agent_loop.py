...
def _starter_agent():
    return create_default_gameops_agent(load_starter_knowledge_base())
...
async def test_agent_answers_chinese_rebate_question_in_chinese_with_sources() -> None:
    ...
    response = await agent.ask(
        GameOpsAskRequest(question="玩家错过了充值返利奖励，客服可以承诺什么？")
    )
    ...
    assert any(
        "玩家 ID" in action or "玩家id" in action.lower() for action in response.next_actions
    )
